...
import com.mongodb.model.PageModel;
 ...
 * @date 2017年4月12日
 */
public abstract class BaseDaoImpl<T> extends IBaseDao<T> {
    ...
    public PageModel<T> pageByProps(int pageNo, int pageSize, String[] params, Object[] values, String order) {
        // 创建分页模型对象
        PageModel<T> page = new PageModel<T>(pageNo, pageSize);
    ...
    protected Query createQuery(String[] params, Object[] values, String order) {
        ...
        // where 条件
        if (!StringUtils.isEmpty(params) && !StringUtils.isEmpty(values)) {
